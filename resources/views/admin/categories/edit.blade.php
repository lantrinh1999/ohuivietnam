        ...
        <form id="add-form" class="form add-form">
            <div class="col-md-6 ">
                <div class="form-group">
                    ...
                    <input value="@if($category->name){{ $category->name }}@endif" class="form-control" type="text" name="name" id="name" placeholder="Tiêu đề">
                    <span id="error-name" class="error error-name text-danger text-bold"></span>
                </div>
                <div class="form-group">
                    <label for="">Slug</label>
                    <input class="form-control" value="@if($category->slug){{ $category->slug }}@endif" type="text" name="slug" id="slug" placeholder="abc-xyz">
...
<script>
    $(function () {
        ...
        function sendAjax() {
            ...
            $.ajax({
                url: "{{ route('admin.categories.saveEdit', ['id' => Request::segment(4)]) }}",
                method: 'POST',
                processData: false,
                contentType: false,
                async: false,
